...
(a)	 Data compile/gathering:  set up DB (SQL or dataframe)? (Loc)

(b)	Blockchain:  Deploy contract, mint token, btc/eth->token
 transfer tokens/eth, balance check, (Leigh)

(c ) Python:  transaction ledger, balance tracking, table for bidder rank

(d) Streamlit:  formatting, input gathering, table/dashboard display, timer  (Eunice)

(e) Presentation Preparation (Amine)
